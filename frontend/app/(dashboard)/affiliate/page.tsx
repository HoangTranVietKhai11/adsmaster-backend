...
export default function AffiliatePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/affiliates/dashboard`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
  ...
  async function joinProgram() {
    setJoining(true);
    const token = localStorage.getItem("accessToken");
    const res = await fetch(`${API_BASE_URL}/api/affiliates/join`, { method: "POST", headers: { Authorization: `Bearer ${token}` } });
  ...
  if (!affiliate) return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-white">Chương trình cộng tác viên</h1>
      <div className="glass-card rounded-2xl p-12 flex flex-col items-center text-center max-w-lg mx-auto">
        ...
        <h2 className="text-2xl font-bold text-white">Nhận 30% hoa hồng trọn đời</h2>
        <p className="text-muted-foreground mt-3 text-sm">Giới thiệu khách hàng mới và nhận 30% phí đăng ký hàng tháng của họ — mãi mãi. Không giới hạn.</p>
        <div className="grid grid-cols-3 gap-4 mt-6 w-full">
          {[["30%", "Hoa hồng"], ["Trọn đời", "Hàng tháng"], ["Không giới hạn", "Thu nhập"]].map(([val, lbl]) => (
        ...
        <button onClick={joinProgram} disabled={joining} className="btn-primary mt-6 flex items-center gap-2">
          ...
          {joining ? "Đang tham gia..." : "Tham gia chương trình ngay"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-white">Bảng điều khiển cộng tác viên</h1>
      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: Users, label: "Tổng lượt giới thiệu", value: stats?.totalReferrals || 0 },
          { icon: DollarSign, label: "Tổng thu nhập", value: `$${(stats?.totalEarned || 0).toFixed(2)}` },
          { icon: TrendingUp, label: "Số dư hiện tại", value: `$${(stats?.balance || 0).toFixed(2)}` },
          { icon: DollarSign, label: "Đã thanh toán", value: `$${(stats?.totalPaid || 0).toFixed(2)}` },
        ].map((s, i) => (
      ...
      <div className="glass-card rounded-2xl p-6">
        <h3 className="font-semibold text-white mb-3 flex items-center gap-2"><Link2 className="w-4 h-4 text-brand-500" /> Link giới thiệu của bạn</h3>
        ...
        <p className="text-xs text-muted-foreground mt-2">Nhận 30% hoa hồng cho mỗi người dùng bạn giới thiệu — hoa hồng trọn đời hàng tháng.</p>
      </div>
    </div>
  );
}
