        ...
        <div className="space-y-6">
            ...
            {stats && (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { icon: Users, label: "Total Users", value: stats.totalUsers, color: "text-brand-500" },
                        { icon: Megaphone, label: "Campaigns", value: stats.totalCampaigns, color: "text-purple-500" },
                        { icon: DollarSign, label: "Total Revenue", value: `$${stats.totalRevenue?.toFixed(0)}`, color: "text-emerald-500" },
                        { icon: UserCheck, label: "Affiliates", value: stats.totalAffiliates, color: "text-pink-500" },
                    ].map((s, i) => (
            ...
            <div className="flex gap-2">
                {(["overview", "users", "payments"] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all capitalize ${tab === t ? "gradient-brand text-white" : "text-muted-foreground hover:text-white hover:bg-white/5"}`}>
                        {t}
                    </button>
                ))}
            </div>
                ...
                <div className="glass-card rounded-xl overflow-hidden">
                    <table className="w-full">
                        <thead><tr className="border-b border-white/5">
                            {["User", "Email", "Role", "Plan", "Campaigns", "Joined"].map(h => (
                            ...
                            {users.map((u, i) => (
                                <motion.tr key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${u.role === "ADMIN" ? "text-red-400 bg-red-400/10" : u.role === "INSTRUCTOR" ? "text-brand-400 bg-brand-400/10" : "text-gray-400 bg-gray-400/10"}`}>{u.role}</span>
                                    </td>
                                    <td className="px-4 py-3 text-sm text-muted-foreground">{u.plan?.name || "FREE"}</td>
                        ...
                        <tbody>
                            {stats.recentPayments.map((p: any, i: number) => (
                                <tr key={p.id} className="border-b border-white/5">
                                    ...
                                    <td className="px-4 py-3"><span className="text-xs text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full">{p.status}</span></td>
